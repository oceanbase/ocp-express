...
public class MysqlInfoAccessor implements InfoAccessor {

    private static final String SELECT_NOW = "select now()";

    private final ObConnectTemplate connectTemplate;

    public MysqlInfoAccessor(ObConnectTemplate connectTemplate) {
        this.connectTemplate = connectTemplate;
    }

    @Override
    public Timestamp now() {
        return connectTemplate.queryForObject(SELECT_NOW, (rs, n) -> rs.getTimestamp(1));
    }
}
