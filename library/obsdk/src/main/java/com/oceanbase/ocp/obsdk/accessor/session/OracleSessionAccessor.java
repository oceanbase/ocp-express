...
@Slf4j
public class OracleSessionAccessor extends AbstractSessionAccessor {
    ...
    private static final String QUERY_PROCESSLIST =
            "SELECT p.*, rownum FROM GV$OB_PROCESSLIST P WHERE 1=1 %s %s";

    public OracleSessionAccessor(ObConnectTemplate connectTemplate) {
    ...
    public List<ObSession> listSession(SessionQueryCondition condition) {
        ...
        String sql = String.format(QUERY_PROCESSLIST, condStr, orderStr);
        if (condition.getPage() != null && condition.getSize() != null) {
            sql = String.format(" select * from(%s) where rownum > %d and rownum <= %d",
                    sql, condition.getPage() * condition.getSize(), (condition.getPage() + 1) * condition.getSize());
        }
