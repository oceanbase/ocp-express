...
public class RolePrivilegeOperation extends PrivilegeOperation {
    ...
    public void operate(UserAccessor userAccessor) {
        if (operationType == PrivilegeOperationType.GRANT) {
            userAccessor.grantRole(grantee.getName(), roles);
        } else {
            userAccessor.revokeRole(grantee.getName(), roles);
        }
    }
