...
public interface TaskInstanceRepository extends JpaRepository<TaskInstanceEntity, Long> {
    ...
    @Override
    @EntityGraph("task.all")
    Optional<TaskInstanceEntity> findById(Long aLong);

    @Query(value = "SELECT u.id FROM TaskInstanceEntity u WHERE u.state = ?1")
    Set<Long> findAllIdByState(TaskState state);
